...
int ext_set_autoexec_query(Json::Value & request, Json::Value & response) 
{
    ...
    for(index = 0; index < queryplan.size(); ++index) 
    {
        ...
        {
            char tmp[DBMT_ERROR_MSG_SIZE]; 
            snprintf(tmp, DBMT_ERROR_MSG_SIZE-1, "Query script too long. MAX_AUTOQUERY_SCRIPT_SIZE:%d.", MAX_AUTOQUERY_SCRIPT_SIZE); 
            tmp_file.close(); 
...
int ext_add_dbmt_user_new(Json::Value &request, Json::Value &response) 
{
    ...
    JSON_FIND_V(request, "authoritylist", build_server_header(response, ERR_PARAM_MISSING, "Parameter(authoritylist) missing in the request")); 
    ...
    if (json_value["admin"] != Json::Value::null) 
    {
        ...
        authinfo = (T_DBMT_USER_AUTHINFO *) increase_capacity (authinfo, sizeof (T_DBMT_USER_AUTHINFO), num_authinfo, num_authinfo + 2);
        if (authinfo == NULL) 
        {
            dbmt_user_free(&dbmt_user); 
    ...
    {
        JSON_FIND_V(json_value, "dbc", build_server_header(response, ERR_PARAM_MISSING, "Parameter(dbc or admin) missing in the authoritylist")); 
        JSON_FIND_V(json_value, "dbo", build_server_header(response, ERR_PARAM_MISSING, "Parameter(dbo or admin) missing in the authoritylist")); 
        JSON_FIND_V(json_value, "brk", build_server_header(response, ERR_PARAM_MISSING, "Parameter(brk or admin) missing in the authoritylist")); 
        JSON_FIND_V(json_value, "mon", build_server_header(response, ERR_PARAM_MISSING, "Parameter(mon or admin) missing in the authoritylist")); 
        JSON_FIND_V(json_value, "job", build_server_header(response, ERR_PARAM_MISSING, "Parameter(job or admin) missing in the authoritylist")); 
        JSON_FIND_V(json_value, "var", build_server_header(response, ERR_PARAM_MISSING, "Parameter(var or admin) missing in the authoritylist")); 
        ...
        authinfo = (T_DBMT_USER_AUTHINFO *) increase_capacity (authinfo, sizeof (T_DBMT_USER_AUTHINFO), num_authinfo, num_authinfo + 7);
        if (authinfo == NULL) 
        {
            dbmt_user_free(&dbmt_user); 
            return build_server_header(response, ERR_MEM_ALLOC, "Memory Allocation error."); 
    ...
    for (unsigned int i = 0; i < dbauthlist.size(); ++i) 
    {
        ...
        JSON_FIND_V(dbauthlist[i], "dbname", build_server_header(response, ERR_PARAM_MISSING, "Parameter(dbname) missing in the authoritylist")); 
        JSON_FIND_V(dbauthlist[i], "dbid", build_server_header(response, ERR_PARAM_MISSING, "Parameter(dbid) missing in the authoritylist")); 
        JSON_FIND_V(dbauthlist[i], "dbpassword", build_server_header(response, ERR_PARAM_MISSING, "Parameter(dbpassword) missing in the authoritylist")); 
        JSON_FIND_V(dbauthlist[i], "dbbrokeraddress", build_server_header(response, ERR_PARAM_MISSING, "Parameter(dbbrokeraddress) missing in the authoritylist"));
    ...
    dbmt_user.user_info = (T_DBMT_USER_INFO *) increase_capacity (dbmt_user.user_info, sizeof (T_DBMT_USER_INFO), num_dbmt_user, num_dbmt_user + 1);
    ...
    num_dbmt_user++; 
    dbmt_user_set_userinfo (&(dbmt_user.user_info[num_dbmt_user-1]), user_id.c_str(), dbmt_password, num_authinfo, authinfo, num_dbinfo, dbinfo); 
...
int ext_set_mon_interval(Json::Value & request, Json::Value & response) 
{
    ...
    if (interval >= MAX_INTERVAL) {
        stringstream sstr;
        sstr << "The interval " << interval << " seconds is bigger than or equal to the maximum interval " << MAX_INTERVAL << " seconds";
        return build_server_header (response, ERR_WITH_MSG, sstr.str().c_str());
    }
