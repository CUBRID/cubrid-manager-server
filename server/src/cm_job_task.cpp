...
tsRenameDB (nvplist *req, nvplist *res, char *_dbmt_error)
{
  ...
  if (is_invalid_filename (exvolpath))
    {
      ERR_FILENAME_NOT_ALLOWED (_dbmt_error, exvolpath);
      return ERR_WITH_MSG;
    }
		  ...
		  char *diag_error)
{
  ...
  FILE *fdRes = NULL, *fdQ = NULL, *fdT = NULL, *fdAnalyzeResult = NULL;
#if defined(WINDOWS)
  DWORD th_id;
#else
  T_THREAD th_id;
...
ts_write_and_save_conf (nvplist *req, nvplist *res, char *_dbmt_error)
{
  ...
  if (is_invalid_filename (conf_path))
    {
      ERR_FILENAME_NOT_ALLOWED (_dbmt_error, conf_path);
      return ERR_WITH_MSG;
    }
...
ts_get_folders_with_keyword (nvplist *req, nvplist *res, char *_dbmt_error)
{
  ...
  if (is_invalid_filename (search_folder))
    {
      ERR_FILENAME_NOT_ALLOWED (_dbmt_error, search_folder);
      return ERR_WITH_MSG;
    }
		       ...
		       int return_dbmt_pwd, char *_dbmt_error)
{
  ...
  for (i = 0; i < dbmt_user->num_dbmt_user; i++)
      ...
      for (j = 0; j < dbmt_user->user_info[i].num_authinfo; j++)
	{
	  ...
	  else if (strcmp (dbmt_user->user_info[i].authinfo[j].domain,
			   "statusmonitorauth") == 0)
	    {
